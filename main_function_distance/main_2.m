function main()
    ...
    if flag
        ...
        elseif dataSwitch == 2
            ...
            % 在数据采集循环之前添加这些变量定义
            accel_range = 2;   % 加速度计量程 ±2g
            gyro_range = 250;  % 陀螺仪量程 ±250dps
            pitch = 0; roll = 0; yaw = 0; % 初始角度值
            
            % 初始化姿态显示
            % 确保正确调用初始化函数并接收所有返回值
            if ~exist('attitudeSphere', 'var')
            ...
            for i = 1:maxPoints
                try
                    ...
                    if mod(i, 10) == 0 || i == 1
                        % 计算当前的角度值（简单示例，您可以根据需要调整）
                        ax_temp = double(accelXData(i));
                    ...
                    yaw = 0; % 偏航角需要磁力计或陀螺仪积分
                        ...
                        updateAttitudeDisplay(attitudeQuiver, attitudeText, attitudeSphere, ...
                            ...
                            attitudeAxes, accel_range, gyro_range, pitch, roll, yaw);
                    end
