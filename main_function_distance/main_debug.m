function main()
    ...
    if flag
        ...
        motion_state = zeros(1, maxPoints); % 0=静止, 1=运动
        % =============================
        ...
        for i = 1:maxPoints
            try
                ...
                if mod(i, 10) == 0 || i == 1
                    % 使用第一次调用calculateAttitude的结果
                    currentPitch = accPitchAngles(i);
                    currentRoll = accRollAngles(i);
                    ...
                    fprintf('姿态角 - 加速度计: pitch=%.1f°, roll=%.1f° | 融合后: pitch=%.1f°, roll=%.1f°\n', ...
                        currentPitch, currentRoll, displayPitch, displayRoll);
