function imu_to_displacement()
    % IMU_TO_DISPLACEMENT 从IMU加速度数据计算位移的完整流程
    ...
    accel_motion(100:200, 1) = 2;       % X轴方向在1-2秒内有 2 m/s? 的加速度
    accel_motion(300:400, 1) = -1.5;    % X轴方向在3-4秒内有 -1.5 m/s? 的减速度
    ...
    velocity(1:50, :) = 0;    % 假设前0.5秒静止
    velocity(450:end, :) = 0; % 假设最后0.5秒静止
    ...
    % 创建一个大图窗
    figure('Position', [100, 100, 1200, 800], 'Name', 'IMU位移解算结果');
    ...
    % 保存图像
    saveas(gcf, 'imu_displacement_results.png');
    fprintf('结果已保存为 imu_displacement_results.png\n');
    ...
    theoretical_x_displacement = trapz(t, trapz(t, accel_motion(:,1)));
    ...
    % 提示用户关于误差的问题
    fprintf('\n注意：由于IMU传感器固有的噪声和偏差，解算结果存在漂移是正常现象。\n');
    fprintf('在实际应用中，需要更高级的算法（如卡尔曼滤波）和传感器融合技术来抑制这种漂移。\n');
end